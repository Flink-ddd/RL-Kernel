#!/usr/bin/env python3
"""Build the matched ROCm G10 P/P vs G11 R/R 200-step result bundle."""
...
def read_log(path: Path, *, arm: str) -> dict[int, dict[str, Any]]:
    ...
    if sorted(rows) != list(range(200)):
        raise RuntimeError(f"{arm} log has {len(rows)} merged steps; expected 0..199")
    return rows
...
def build_rows(
    g10: dict[int, dict[str, Any]], g11: dict[int, dict[str, Any]]
    ...
    aliases = {
        ...
        "log_probs_time_s": "perf/log_probs_time",
        ...
        "mean_logp_diff": "train/train_rollout_logprob_abs_diff",
    ...
    for index in range(200):
        ...
        for metric, log_key in aliases.items():
            ...
            if log_key not in source10:
                raise RuntimeError(f"missing G10 {log_key} at step {index + 1}")
            row[f"g10_{metric}"] = finite(source10[log_key], field=log_key, step=index)
        for metric in aliases:
            row[f"delta_g11_minus_g10_{metric}"] = float(row[f"g11_{metric}"]) - float(
                row[f"g10_{metric}"]
            )

        # Preserve every scalar RL/performance field, including metrics not plotted.
        scalar_keys = sorted(set(source10) | set(source11))
        for key in scalar_keys:
            csv_key = re.sub(r"[^a-zA-Z0-9]+", "_", key).strip("_")
            for arm, source in (("g10", source10), ("g11", source11)):
                value = source.get(key)
                if isinstance(value, (int, float)) and math.isfinite(float(value)):
                    row[f"{arm}_raw_{csv_key}"] = float(value)
        result.append(row)
    return result
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    def normalized(subset: list[dict[str, Any]]) -> dict[str, Any]:
        result: dict[str, Any] = {"n": len(subset)}
        for group in ("g10", "g11"):
            response_tokens = values(subset, f"{group}_response_len_mean_tokens")
            result[group] = {
                "mean_response_tokens": float(response_tokens.mean()),
                "rollout_tokens_per_gpu_s": float(
                    response_tokens.sum() / values(subset, f"{group}_rollout_time_s").sum()
                ),
                "reference_logp_tokens_per_gpu_s": float(
                    response_tokens.sum() / values(subset, f"{group}_ref_log_probs_time_s").sum()
                ),
                "actor_logp_tokens_per_gpu_s": float(
                    response_tokens.sum() / values(subset, f"{group}_log_probs_time_s").sum()
                ),
                "actor_train_tokens_s": float(
                    8 * response_tokens.sum() / values(subset, f"{group}_actor_train_time_s").sum()
                ),
                "train_tokens_per_gpu_s": float(
                    response_tokens.sum() / values(subset, f"{group}_train_time_s").sum()
                ),
                "end_to_end_tokens_per_gpu_s": float(
                    response_tokens.sum() / values(subset, f"{group}_step_time_s").sum()
                ),
                "mean_step_time_s": float(values(subset, f"{group}_step_time_s").mean()),
            }
        return result

    fair_rows = rows[1:]
    fair = normalized(fair_rows)
    fair["excluded_log_step_indices"] = [0]
    full_length_rows = [
        row
        for row in fair_rows
        if row["g10_response_len_mean_tokens"] == 7168
        and row["g11_response_len_mean_tokens"] == 7168
    ]
    full_length = normalized(full_length_rows)
    full_length["log_step_indices"] = [int(row["log_step_index"]) for row in full_length_rows]

    specs = {
        "step_time_s": False,
        "rollout_time_s": False,
        "tokens_per_gpu_s": True,
        "ref_log_probs_time_s": False,
        "actor_train_time_s": False,
        "actor_train_tokens_s": True,
        "train_time_s": False,
    }
    rng = np.random.default_rng(1234)
    ...
    for metric, higher_is_better in specs.items():
        ...
        effects[metric] = {
            ...
            "positive_means_g11_better": True,
        }
    return {
        "paired": paired,
        "token_normalized_steps_1_199": fair,
        "equal_length_7168_steps": full_length,
        "performance_effect_pct": effects,
        "bootstrap": {
        ...
        "formulas": {
            "rollout_tokens_per_gpu_s": (
                "mean response tokens * 8 samples / (rollout seconds * 8 GPUs)"
            ),
...
def plot_performance_summary(rows: list[dict[str, Any]], output_dir: Path) -> None:
    ...
    headline = (
        ...
        f"G11 end-to-end delta: {100 * (1 - g11_time[4] / g10_time[4]):+.1f}%\n\n"
        ...
        "1 node · 8×MI300X 192GB · TP4/CP2 · batch 8"
    )
    axes[1, 1].text(
        0.04,
        0.93,
    ...
    fig.suptitle("VIME Qwen3-8B on ROCm · G10 vs G11", fontsize=19, fontweight="bold", y=0.995)
...
def plot_performance_matrix(rows: list[dict[str, Any]], output_dir: Path) -> None:
    ...
    def finding(metric: str, *, higher_is_better: bool) -> str:
        g10, g11 = mean("g10", metric), mean("g11", metric)
        if math.isclose(g10, g11, rel_tol=0.005):
            return "Parity"
        g11_wins = (g11 > g10) == higher_is_better
        direction = "higher" if higher_is_better else "faster"
        return f"{'G11' if g11_wins else 'G10'} {direction}"
    ...
    performance = [
        [
            ...
            finding("rollout_time_s", higher_is_better=False),
        ],
        [
            "Rollout throughput",
            "tok/GPU/s",
            ...
            finding("tokens_per_gpu_s", higher_is_better=True),
        ],
        [
            "Reference logp",
            "s / step",
            ...
            finding("ref_log_probs_time_s", higher_is_better=False),
        ],
        [
            "Actor train",
            "s / step",
            ...
            finding("actor_train_time_s", higher_is_better=False),
        ],
        [
            "Actor throughput",
            "tok/s",
            ...
            finding("actor_train_tokens_s", higher_is_better=True),
        ],
        [
            "Total train",
            "s / step",
            ...
            finding("train_time_s", higher_is_better=False),
        ],
        [
            "End-to-end step",
            "s / step",
            ...
            finding("step_time_s", higher_is_better=False),
        ],
    ]
    correctness = [
        [
    ...
    columns = ["Metric", "Unit", "G10 P/P", "G11 R/R", "G11 vs G10", "Finding"]
    fig = plt.figure(figsize=(15.2, 9.3), facecolor="#F7E8D2")
    fig.text(
        0.5,
        0.965,
        "VIME Qwen3-8B on ROCm · 200-Step Performance Matrix",
        ha="center",
        va="center",
        fontsize=20,
        fontweight="bold",
    ...
    fig.text(
        0.5,
        0.905,
        "Matched workload · 1 node · 8× AMD MI300X 192GB · TP4/CP2 · global batch 8 · seed 1234",
        ha="center",
        va="center",
        fontsize=12.5,
        fontweight="bold",
...
def plot_performance_trajectories(rows: list[dict[str, Any]], output_dir: Path) -> None:
    ...
    fig.suptitle(
        "G10 P/P vs G11 R/R on ROCm · Performance Across 200 Steps",
        fontsize=18,
        fontweight="bold",
        y=0.995,
    )
...
def plot_consistency_reward(rows: list[dict[str, Any]], output_dir: Path) -> None:
    ...
    fig.suptitle(
        "G10 P/P vs G11 R/R on ROCm · Training and Bitwise Consistency",
        fontsize=18,
        fontweight="bold",
        y=0.995,
    )
...
def plot_mean_logp_diff(rows: list[dict[str, Any]], output_dir: Path) -> None:
    steps = np.arange(1, 201)
    fig, axis = plt.subplots(figsize=(14, 7.2))
    for group, raw, strong in (("G10", LIGHT_RED, RED), ("G11", LIGHT_BLUE, BLUE)):
        series = values(rows, f"{group.lower()}_mean_logp_diff")
        axis.plot(steps, series, color=raw, linewidth=0.9, alpha=0.7)
        axis.plot(
            steps,
            moving_average(series),
            color=strong,
    ...
    fig.text(
        0.5,
        0.02,
        "G10 production P/P vs G11 strict R/R · Qwen3-8B · ROCm MI300X",
        ha="center",
        fontsize=10,
        color="#4B525B",
    )
    fig.tight_layout(rect=(0, 0.05, 1, 1))
    save(fig, output_dir, "mean-logp-diff")


def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    first = ["step", "log_step_index"]
    fieldnames = first + sorted(set().union(*(row.keys() for row in rows)) - set(first))
...
def write_readme(path: Path, summary: dict[str, Any]) -> None:
    ...
    ref_logp_delta = 100 * (1 - ref_logp["g11"]["mean"] / ref_logp["g10"]["mean"])
    actor_delta = 100 * (1 - actor["g11"]["mean"] / actor["g10"]["mean"])
    step_delta = 100 * (1 - step_time["g11"]["mean"] / step_time["g10"]["mean"])

    def result(value: float, positive: str, negative: str) -> str:
        return f"G11 {abs(value):.1f}% {positive if value >= 0 else negative}"

    table_rows = "\n".join(
        [
            (
                f"| Rollout time (s) | {rollout['g10']['mean']:.2f} | "
                f"{rollout['g11']['mean']:.2f} | {result(rollout_gain, 'faster', 'slower')} |"
            ),
            (
                f"| Rollout tokens/GPU/s | {rollout_tput['g10']['mean']:.2f} | "
                f"{rollout_tput['g11']['mean']:.2f} | "
                f"{result(rollout_tput_gain, 'higher', 'lower')} |"
            ),
            (
                f"| Reference logp time (s) | {ref_logp['g10']['mean']:.2f} | "
                f"{ref_logp['g11']['mean']:.2f} | "
                f"{result(ref_logp_delta, 'faster', 'slower')} |"
            ),
            (
                f"| Actor train time (s) | {actor['g10']['mean']:.2f} | "
                f"{actor['g11']['mean']:.2f} | {result(actor_delta, 'faster', 'slower')} |"
            ),
            (
                f"| Total step time (s) | {step_time['g10']['mean']:.2f} | "
                f"{step_time['g11']['mean']:.2f} | {result(step_delta, 'faster', 'slower')} |"
            ),
            (
    ...
    fair = summary["token_normalized_steps_1_199"]
    text = f"""# ROCm G10 P/P vs G11 R/R (200 steps)
...
{fair["g11"]["end_to_end_tokens_per_gpu_s"]:.2f} tok/GPU/s for G11 and
{fair["g10"]["end_to_end_tokens_per_gpu_s"]:.2f} tok/GPU/s for G10.
...
def main() -> None:
    ...
    rows = build_rows(
        read_log(args.g10_log, arm="G10"),
        read_log(args.g11_log, arm="G11"),
    )
